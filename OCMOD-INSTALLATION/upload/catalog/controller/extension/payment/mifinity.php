...
//		$data['PostalCode'] = (string)substr($order_info['payment_postcode'], 0, 30);
		...
		$data['phone'] = (string)substr($order_info['telephone'], 0, 32);
...
//        $request->processors = array("NXAQCR","NXEWALLET");
        ...
        $request->client->phone = (string)substr($order_info['telephone'], 0, 32);
...
//		$request->client->State = (string)substr($order_info['shipping_zone'], 0, 50);
		$request->address->postalCode = (string)substr($order_info['shipping_postcode'], 0, 30);
...
//			$request->CustomerReadOnly = true;
			$result = $this->model_extension_extension->initIframe($request);
...
//			$result = $this->model_extension_payment_mifinity->getAccessCode($request);
		...
		if (isset($lbl_error)) {
			...
			if ($this->config->get('mifinity_paymode') == 'iframe') {
				$data['accountHolderId'] = $this->config->get('mifinity_account_holder_id');
