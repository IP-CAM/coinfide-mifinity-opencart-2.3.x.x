...
class ModelExtensionPaymentMiFinity extends Model {
	...
	public function getAuth($request) {
		if ($this->config->get('mifinity_test')) {
			$url = 'https://demo.mifinitypay.com/api/oauth/token';
		} else {
			$url = 'https://secure.mifinitypay.com/api/oauth/token';
		}
...
//		$response = json_decode($response);
	...
	public function initIframe($request,$accountHolderId) {
//        $mifinity_accountHolderId = html_entity_decode($this->config->get('mifinity_account_holder_id'), ENT_QUOTES, 'UTF-8');
        $mifinity_accountHolderId = html_entity_decode($accountHolderId, ENT_QUOTES, 'UTF-8');
		if ($this->config->get('mifinity_test')) {
		...
		$response = $this->sendCurlJson($url, $request);
//		$response = json_decode($response);

		return $response;
	}

	public function getStatusPayment($validationKey,$accountHolderId) {
//        $mifinity_accountHolderId = html_entity_decode($this->config->get('mifinity_account_holder_id'), ENT_QUOTES, 'UTF-8');
        $mifinity_accountHolderId = html_entity_decode($accountHolderId, ENT_QUOTES, 'UTF-8');
		...
		$response = $this->sendCurlJson($url, '', false);
//		$response = json_decode($response);

		return $response;
	}

	public function sendCurlJson($url, $data, $is_post=true) {
    $ch = curl_init($url);
...
//        print_r(json_decode($response));
    ...
    if (curl_errno($ch) != CURLE_OK) {
        ...
        if ($info['http_code'] != 200 && $info['http_code'] != 201) {
            $response = json_decode($response);
            $response->errors = 'Error connecting to MiFinity: ' . $info['http_code'] .' || '. json_encode($response).'||'.json_encode($data);
    ...
    public function sendCurl($url, $data, $is_post=true) {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: application/x-www-form-urlencoded","api-version: 1"));
		curl_setopt($ch, CURLOPT_USERPWD, $mifinity_username . ":" . $mifinity_password);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        if ($is_post) {
            curl_setopt($ch, CURLOPT_POST, true);
